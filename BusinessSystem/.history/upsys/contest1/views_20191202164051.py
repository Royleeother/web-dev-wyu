...
class InfoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        request_user = self.request.user.student.SID
        request_form_sid = int(self.request.path.split('/')[3])
        # contest_user = Contest1(form)
        # print("康康form:", form)
        print("request:", type(request_form_sid))
        print("request:", request_form_sid)
