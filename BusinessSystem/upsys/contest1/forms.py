...
class Contest1Form(ModelForm):
    ...
    CHOICES = (('舞厅', '舞厅'),('多功能厅', '多功能厅'),('报告厅', '报告厅'),)
    apply_place = forms.ChoiceField(label='申请地点', choices=CHOICES, 
                    widget=forms.Select(attrs={'class': 'form-control',
                                            }))
    apply_place_number = forms.CharField(label='厅室编号', 
                    widget=forms.TextInput(attrs={'placeholder': '如：202', 
                                            'class': 'form-control',
                                            }))                                        
...
class Contest1_for_review(forms.Form):
    def __init__(self, *args, **kwargs):
        review_type = kwargs.pop('review_type')  # <-- getting the value from keyword arguments
        print("review_type in form :", review_type)
        super().__init__(*args, **kwargs)

        YES_NO = ((True, '是'), (False, '否'))
        self.fields[f'{review_type}_decision'] =forms.BooleanField(label='是否审核通过',required=False,
                                        widget=forms.RadioSelect(
                                        choices=YES_NO, 
                                        attrs={'class': 'form-control', 
                                                'required':True,
                            }))
        # self.fields[f'{review_type}_decision'] = forms.RadioSelect(
        #                                 choices=YES_NO, 
        #                                 attrs={'class': 'form-control', 
        #                                             'required':True,
        #                                             })
        self.fields[f'{review_type}_comment'] = forms.CharField(label='留言', 
    ...
    # class Meta:
    #     fields = [
    #         'guiding_unit_comment',
    #         'discipline_unit_comment',
    #         'ccyl_comment',
    #         'guiding_unit_decision',
    #         'discipline_unit_decision',
    #         'ccyl_decision'
    #     ]
